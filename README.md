...
`data` - Необязательный параметр. Данные таблцы. Тип: (string | number)[][].
### Пример:
...
const table = new Table(); // Создает таблицу 1 на 1
table.setData(tableArr); // Добавляет данные в таблицу
```
